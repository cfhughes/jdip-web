	...
	*/
	...
	throws javax.xml.parsers.ParserConfigurationException, NoVariantsException
	{
		...
		if(vm.isInWebstart)
		{
			...
			try
			{
				enum1 = cl.getResources(VARIANT_FILE_NAME);
			}
			catch(IOException e)
			{
				enum1 = null;
			...
			if(enum1 != null)
			{
				...
				{
					URL variantURL = (URL) enum1.nextElement();
		...
		if(vm.isInWebstart)
		{
			...
			try
			{
				enum1 = cl.getResources(SYMBOL_FILE_NAME);
			}
			catch(IOException e)
			{
				enum1 = null;
			...
			if(enum1 != null)
			{
				while(enum1.hasMoreElements())
				{
					URL symbolURL = (URL) enum1.nextElement();
	...
	*/
	private static URL getWSResource(MapRecObj mro, URI uri)
	{
		...
		if(mro != null)
		{
			...
			try
			{
				enum1 = cl.getResources(uri.toString());
			}
			catch(IOException e)
			{
				return null;
